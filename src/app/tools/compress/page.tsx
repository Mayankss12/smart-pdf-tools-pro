        ...
        <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="overflow-hidden rounded-[2rem] border border-violet-100 bg-white shadow-[0_18px_50px_rgba(91,63,193,0.08)]">
            ...
            <section className="grid gap-0 lg:grid-cols-[1fr_400px]">
                  ...
                  <div className="mt-5 rounded-[1.5rem] border border-violet-100 bg-white p-5 shadow-sm">
                    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                      ...
                      <button type="button" onClick={clearFile}
                        className="inline-flex items-center gap-2 rounded-full border border-red-100 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-100 transition">
                        <X size={15} /> Remove
                      </button>
                    </div>
                  </div>
              ...
              <aside className="bg-white p-5 sm:p-6 space-y-4">

                {/* Level selector */}
                <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                  <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Compression level</h2>
                ...
                {/* Target size */}
                <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                  <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Target file size</h2>
                  <select
                    value={targetBytes ?? ""}
                ...
                {/* Summary */}
                <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5">
                  <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Summary</h2>
                  <div className="grid gap-3">
                    {[
                      { label: "Original",  val: file ? formatFileSize(file.size) : "—" },
                      { label: "Output",    val: result ? formatFileSize(result.compressedSize) : "—" },
                      { label: "Reduction", val: result ? `${result.savedPercent}%` : "—", green: result && result.savedPercent > 0 },
                    ].map(({ label, val, green }) => (
                      <div key={label} className="rounded-2xl bg-white p-4">
                        <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</div>
                        <div className={`mt-1 text-xl font-semibold ${green ? "text-emerald-600" : "text-slate-950"}`}>{val}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {isLoading && (
                    <div className="mt-4 overflow-hidden rounded-full bg-slate-200">
                      <div className="h-1.5 rounded-full bg-indigo-600 transition-all duration-300" style={{ width: `${progress}%` }} />
                    </div>
                  )}
                ...
                {/* Status */}
                <div className={`rounded-[1.5rem] border p-4 text-sm font-medium leading-6 ${
