...
function findEndRecord(bytes: Uint8Array) {
  ...
  throw new PdfEngineError("INVALID_FILE", "This Office file does not contain a valid ZIP directory.");
}
...
function readZipRecords(bytes: Uint8Array) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (view.getUint32(offset, true) !== 0x02014b50) {
      throw new PdfEngineError("INVALID_FILE", "The Office ZIP directory is damaged.");
    }
...
async function readZipEntry(
  ...
  if (view.getUint32(record.localOffset, true) !== 0x04034b50) {
    throw new PdfEngineError("INVALID_FILE", `The Office entry ${record.name} is damaged.`);
  }
  ...
  const compressed = bytes.subarray(dataOffset, dataOffset + record.compressedSize);
  ...
  if (record.compression === 8) {
    ...
    if (record.uncompressedSize && output.length !== record.uncompressedSize) {
      throw new PdfEngineError("INVALID_FILE", `The Office entry ${record.name} has an invalid size.`);
    }
    return output;
  }
  throw new PdfEngineError(
...
async function extractDocx(
  ...
  if (!record) throw new PdfEngineError("INVALID_FILE", "This DOCX has no word/document.xml part.");
  const document = parseXml(await readZipEntry(bytes, record), "Word document");
...
async function getSharedStrings(
  ...
  const document = parseXml(await readZipEntry(bytes, record), "Excel shared strings");
  const namespace = "http://schemas.openxmlformats.org/spreadsheetml/2006/main";
  return Array.from(document.getElementsByTagNameNS(namespace, "si")).map((item) =>
...
async function extractXlsx(
  ...
  const sheetPaths = sortNumberedPaths(
    [...records.keys()].filter((path) => /^xl\/worksheets\/sheet\d+\.xml$/.test(path)),
  );
  if (!sheetPaths.length) throw new PdfEngineError("INVALID_FILE", "This XLSX contains no worksheets.");
  const sections: string[] = [];
  for (let sheetIndex = 0; sheetIndex < sheetPaths.length; sheetIndex += 1) {
    const record = records.get(sheetPaths[sheetIndex]);
    if (!record) continue;
    const document = parseXml(await readZipEntry(bytes, record), `Excel sheet ${sheetIndex + 1}`);
    const rows = Array.from(document.getElementsByTagNameNS(namespace, "row"));
    const lines = rows.map((row) => {
      const cells = Array.from(row.getElementsByTagNameNS(namespace, "c"));
      return cells
        .map((cell) => {
          const type = cell.getAttribute("t");
          if (type === "inlineStr") {
            return Array.from(cell.getElementsByTagNameNS(namespace, "t"))
              .map((node) => node.textContent ?? "")
              .join("");
          }
          const raw = cell.getElementsByTagNameNS(namespace, "v")[0]?.textContent ?? "";
    ...
    sections.push(`Worksheet ${sheetIndex + 1}\n${lines.join("\n")}`.trim());
  }
...
async function extractPptx(
  ...
  const slidePaths = sortNumberedPaths(
    [...records.keys()].filter((path) => /^ppt\/slides\/slide\d+\.xml$/.test(path)),
  );
  if (!slidePaths.length) throw new PdfEngineError("INVALID_FILE", "This PPTX contains no slides.");
  const sections: string[] = [];
  for (let slideIndex = 0; slideIndex < slidePaths.length; slideIndex += 1) {
    const record = records.get(slidePaths[slideIndex]);
    if (!record) continue;
    const document = parseXml(await readZipEntry(bytes, record), `PowerPoint slide ${slideIndex + 1}`);
    const paragraphs = Array.from(document.getElementsByTagNameNS(drawingNamespace, "p"));
    const lines = paragraphs
      .map((paragraph) =>
        Array.from(paragraph.getElementsByTagNameNS(drawingNamespace, "t"))
          .map((node) => node.textContent ?? "")
          .join(""),
      )
      .filter(Boolean);
...
export async function extractOfficeText(
  ...
  if (file.size > 60 * 1024 * 1024) {
    throw new PdfEngineError("INVALID_FILE", "Office files are limited to 60 MB for browser conversion.");
  }
  const bytes = new Uint8Array(await file.arrayBuffer());
  if (bytes.length < 4 || bytes[0] !== 0x50 || bytes[1] !== 0x4b) {
    throw new PdfEngineError("INVALID_FILE", "This is not a valid Office Open XML package.");
  }
  const records = readZipRecords(bytes);
  if ([...records.keys()].some((path) => /vbaProject\.bin$/i.test(path))) {
    throw new PdfEngineError("INVALID_FILE", "Macro-enabled Office documents are not supported.");
  }
  const text =
    format === "docx"
