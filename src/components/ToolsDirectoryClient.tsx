...
function getVisibleTools(launchReadyToolIds: readonly string[]) {
  const readyIds = new Set(launchReadyToolIds);
  return tools
    .filter((tool) => readyIds.has(tool.id))
...
export function ToolsDirectoryClient({
  ...
  const visibleTools = useMemo(
    () => getVisibleTools(launchReadyToolIds),
    [launchReadyToolIds],
  );
