...
export default function EditorPage() {
  ...
  useEffect(() => {
    async function loadTextOverlay() {
      if (!pdfDocRef.current || (activeTool !== "highlight" && activeTool !== "select")) {
        setTextOverlay([]);
        return;
      }
      ...
      const spans = textContent.items
        .map((item) => {
          ...
          return { text: textItem.str, left, top, width: textItem.width * viewerScale, height: textItem.height };
        })
        .filter((s) => s.text.trim().length > 0);
      setTextOverlay(spans);
    }
    loadTextOverlay();
  }, [activeTool, currentPage, viewerScale]);
  ...
  function enableHighlightMode() {
    ...
    setStatus("Highlight mode active. Select PDF text to highlight.");
  }
